import { GaussianRNG } from './mathUtils';
...
export class WebCNN {
    ...
    static readonly LAYER_TYPE_MAX_POOL =  'maxPoolLayer';
    static readonly LAYER_TYPE_CONV =  'convLayer';
    static readonly LAYER_TYPE_INPUT_IMAGE =  'inputImageLayer';
    static readonly LAYER_TYPE_INPUT =  'inputLayer';
    static readonly LAYER_TYPE_FULLY_CONNECTED =  'FCLayer';
    static readonly ACTIVATION_NONE =  'none';
    static readonly ACTIVATION_RELU =  'relu';
    static readonly ACTIVATION_TANH =  'tanh';
    static readonly ACTIVATION_LINEAR =  'linear';
    static readonly ACTIVATION_SOFTMAX=  'softmax';
    static readonly INIT_ZEROS =  'initZeros';
    static readonly INIT_XAVIER =  'xavier';
    static readonly INIT_GLOROT_UNIFORM =  'glorot_uniform';
    
    static readonly FORWARD_MODE_TRAINING =  'training';		// Normal training mode
    static readonly FORWARD_MODE_EVALUATE =  'evaluate';		// Normal evaluation mode where no values need to be calculated for backprop
    static readonly BACKWARD_MODE_TRAINING =  'training';
    static readonly BACKWARD_MODE_DREAMING =  'dreaming';

    private layers: Array<Layer>;
    private nextLayerIndex: number;
    private forwardMode: string;
    private backwardMode: string;
    private learningRate: number;
    private momentum: number;
    private lambda: number;
    private solutionEpoch: number;
    private trainingError: number;
    private forwardTime: number;
    private backwardTime: number;
    private miniBatchSize: number;
    private batchLearningRate: number;
    private targetValues: Array<any>;
	...
	constructor()
	{
		...
		this.forwardMode = WebCNN.FORWARD_MODE_TRAINING;
		this.backwardMode = WebCNN.BACKWARD_MODE_TRAINING;
	...
	newLayer( layerDesc )
	{
		...
		switch( layerDesc.type )
		{
			...
			case WebCNN.LAYER_TYPE_MAX_POOL:
			{
			...
			case WebCNN.LAYER_TYPE_FULLY_CONNECTED:
			{
		...
		{
			if ( newLayer.type != WebCNN.LAYER_TYPE_INPUT_IMAGE && newLayer.type != WebCNN.LAYER_TYPE_INPUT )
			{
	...
	// intention: public API
	classifyImages( imageDataArray ): Array<Array3D> {
		...
		this.forwardMode = WebCNN.FORWARD_MODE_EVALUATE;
    ...
    static loadNetworkFromJSON( networkJSON ): WebCNN {
        let cnn: WebCNN = new WebCNN();
...
abstract class Layer
{
    ...
    output: Array<Array3D>;
    network: any;
    backpropError: Array<Array3D>;
    biases: Float32Array;
    weights: Array<Array3D>;
    biasVelocities: Float32Array;
    weightVelocities: Array<Array3D>;
    input: any;
	...
	constructor( name, units )
	{
		...
		// Defaults
		this.activation = WebCNN.ACTIVATION_LINEAR;
	}
	...
	abstract forward(input?: any);

	abstract backward();

	abstract setWeightsAndBiases(weights: any, biases: any);
}
...
class InputLayer extends Layer
{
	...
	{
		super( name, units );
		this.type = WebCNN.LAYER_TYPE_INPUT;
		this.activation = WebCNN.ACTIVATION_NONE;
	...
	forward()
	{
		...
		{
			this.output[ example ] = new Array3D( this.outputDimensions, WebCNN.INIT_ZEROS );
	...
	setWeightsAndBiases(weights: any, biases: any) {}
}
...
class InputImageLayer extends Layer
{
	constructor( layerName, imageWidth, imageHeight, imageDepth )
	{
		...
		super( layerName, 1 );
		this.type = WebCNN.LAYER_TYPE_INPUT_IMAGE;
...
class FCLayer extends Layer
{
	...
	{
		super( name, units );
		this.type = WebCNN.LAYER_TYPE_FULLY_CONNECTED;
	...
	setInputLayer( inputLayer )
	{
		...
		{
			this.weights[ unit ] = new Array3D( this.inputDimensions, WebCNN.INIT_XAVIER );
			this.weightVelocities[ unit ] = new Array3D( this.inputDimensions, WebCNN.INIT_ZEROS );
	...
	backward()
	{
		...
		{
			this.backpropError[ example ] = new Array3D( this.inputDimensions, WebCNN.INIT_ZEROS );
			...
			for ( unit = 0; unit < this.units; ++unit )
			{
				...
				if ( this.activation == WebCNN.ACTIVATION_TANH )
				{
...
class ConvLayer extends Layer
{
    ...
    kernelHeight: number;
    kernelDepth: number;
    kernelStrideX: number;
    kernelStrideY: number;
    padX: number;
    padY: number;
    kernels: Array<Array3D>;
    errorDeltas: any;
    kernelVelocities: any;

	constructor( name, units, kernelWidth, kernelHeight, kernelStrideX, kernelStrideY, usePadding )
	{
		super( name, units );

		this.type = WebCNN.LAYER_TYPE_CONV;
		this.activation = WebCNN.ACTIVATION_RELU;
	...
	setInputLayer( inputLayer )
	{
		...
		for ( var unit = 0; unit < this.units; ++unit )
		{
			this.kernels[ unit ] = new Array3D( kernelDimensions, WebCNN.INIT_XAVIER );
			this.kernelVelocities[ unit ] = new Array3D( kernelDimensions, WebCNN.INIT_ZEROS );
		}
	}
...
class MaxPoolLayer extends Layer
{
    ...
    outputHeight: number;
    outputDepth: number;
    poolMaxActivationIndices: Array<UIntCube>;

	constructor( name, poolWidth, poolHeight, strideX, strideY )
	{
		super( name, 0 );
		this.type = WebCNN.LAYER_TYPE_MAX_POOL;
			...
			for ( d = 0; d < this.outputDepth; ++d )
			{
				for ( y = 0; y < this.outputHeight; ++y )
				{
					for ( x = 0; x < this.outputWidth; ++x )
					{
						...
						if ( this.network.forwardMode == WebCNN.FORWARD_MODE_TRAINING )
						{
							bitfield = 0;
...
export class Array3D {
	...
	constructor( dimensions, initType )
	{
		var i;
		...
		if ( initType == WebCNN.INIT_XAVIER )
		{
		...
		else if ( initType == WebCNN.INIT_GLOROT_UNIFORM )
		{
			for ( i = 0; i < this.size; ++i )
			{
	...
	applyActivationFunction( activationType )
	{
		...
		switch( activationType )
		{
			...
			case WebCNN.ACTIVATION_SOFTMAX:
			{
				var max = 0;
				var sumExp = 0;
